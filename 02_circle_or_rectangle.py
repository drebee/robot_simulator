...
if shape == "circle":
    draw_circle(19, True)
    # draw_circle(19, False)
elif shape == "rectangle":
    draw_rectangle(7, 3)

def too_close_to_move_forward(left_dist, right_dist):
    close_dist = 3
    return left_dist <= close_dist or right_dist <= close_dist
...
seconds = int(input("How long would you like me to do this? (in seconds) "))
if first_move == "forward":
    while seconds > 0.1:
        robot.motors(1,1,0.1)            
        left_dist = robot.left_sonar()
        right_dist = robot.right_sonar()
        seconds -= 0.1
