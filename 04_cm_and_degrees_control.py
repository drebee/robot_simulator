...
ratios = [0.016666666666666666,1/.98/60]#0.012018542928131136] #[seconds per mm, #seconds per degree]
...
def calibrate(sonarsDisplacement):
    ...
    right = robot.right_sonar()
    robot.motors(1,1,1)
    newLeft = robot.left_sonar()
    newRight = robot.right_sonar()
...
commandsBasic=['forward','left','right','back',#'help',
               'quit']
...
while Athena == 'the best':
    ...
    if command == 'help':
        ...
        distance = float(ask('How far? (in cm)'))
        forward(distance)
    elif command == 'back':
        d = float(ask('How far? (in cm)'))
        back(d)
    elif command == 'left':
        angle=float(ask('What angle? (in degrees)'))
        turnLeft(angle)
    elif command == 'right':
        angle=float(ask('What angle? (in degrees)'))
        turnRight(angle)
